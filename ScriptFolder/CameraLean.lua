...
This is poorly optimized, if you want to edit it. Do at your will.
...
local function Delta(deltaTime)
	if Humanoid.Health == 0 then bobbing:Disconnect() return end
	...
	if deltaTime == 1.5 then 
		...
		func1 = lerp(func1, math.cos(os.clock() * 0.5 * math.random(5, 7.5)) * (math.random(2.5, 10) / 100) * deltaTime, 0.05 * deltaTime)
		func2 = lerp(func2, math.cos(os.clock() * 0.5 * math.random(2.5, 5)) * (math.random(1, 5) / 100) * deltaTime, 0.05 * deltaTime)
	end
	...
	func4 = lerp(func4, math.sin(os.clock() * int) / 5 * math.min(1, int2 / 10), 0.25 * deltaTime)
	
	----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	
	if rootMagnitude == 1 then
		val = lerp(val, math.cos(os.clock() * 0.5 * math.floor(int)) * (int / 200), 0.25 * deltaTime)
	else
...
bobbing = game:GetService("RunService").RenderStepped:Connect(Delta)
